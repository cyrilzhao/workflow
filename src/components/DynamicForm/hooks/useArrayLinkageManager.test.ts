...
describe('useArrayLinkageManager - 路径映射支持', () => {
  it('应该支持带有路径映射的数组联动', async () => {
    const { result } = renderHook(() => {
      const form = useForm({
        defaultValues: {
          'group~~category~~contacts': [{ type: 'work', 'info~~companyName': '' }],
        },
      });
    ...
    // 初始状态：type = 'work'，companyName 应该显示
    await waitFor(() => {
      expect(
        result.current.linkageStates['group~~category~~contacts.0.info~~companyName']?.visible
      ).toBe(true);
    });

    // 修改类型为 'personal'
    result.current.form.setValue('group~~category~~contacts.0.type', 'personal');
